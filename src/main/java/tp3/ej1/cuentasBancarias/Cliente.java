...
public abstract class Cliente implements OperacionBanco {
  ...
  // Metodos de la interface OperacionBanco (obtenerSaldoDisponible es cambiar el nombre a una que ya esta)
  @Override
  public double obtenerSaldoDisponible() {
    return saldoDisponibleTotal(); // "renombramos" el metodo para cumplir con la interface
  }
  ...
  public double obtenerComision() {
    double total = 0;
    for (int i = 0; i < getCantidadCuentas(); i++) {
      total += getCuentas()[i].obtenerComision();
    }
    return total;
  }
}
